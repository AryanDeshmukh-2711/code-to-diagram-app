...
def _running_header_and_footer(
    ...
    for index, section in enumerate(document.sections):
        ...
        paragraph.text = template.header_text.format(project=project)
...
def _caption(
    document, bookmarks: "_Bookmarks", kind: str, number: int | None, text: str, target_id: str
) -> None:
...
def _add_figure(
    ...
    _caption(document, bookmarks, "Figure", block.number, block.caption, block.figure_id)


def _add_table(document, bookmarks: "_Bookmarks", block: Table, template: DocumentTemplate) -> None:
    _caption(document, bookmarks, "Table", block.number, block.caption, block.table_id)
...
def _add_index(document, entries, title: str, instruction: str) -> None:
...
def _write_section(
    ...
    bookmarks.wrap(
        ...
        lambda: heading.add_run(f"{section.number} {section.title}"),
    )
...
def _write_block(document, bookmarks: "_Bookmarks", block, template: DocumentTemplate) -> None:
    if isinstance(block, Paragraph):
        ...
        _add_index(document, block.entries, "Contents", 'TOC \\o "1-3" \\h \\z \\u')
    elif isinstance(block, ListOfFigures):
        _add_index(document, block.entries, "List of Figures", 'TOC \\h \\z \\c "Figure"')
    elif isinstance(block, ListOfTables):
        _add_index(document, block.entries, "List of Tables", 'TOC \\h \\z \\c "Table"')
